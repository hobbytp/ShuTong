...
### 全局Chatbot
当前的Chatbot是在Pulse下面，但是其实它是个全局的，所以，把它移到右下角作为一个全局可见的chatbot更合理。

### Pulse更主动
Pulse能一直产生新想法，比如当发现用户在某个主题上花了很多时间，那么就在后台做一些扩展性研究，有或者用户在chatbot里面提出了一个想法，那么该chatbot会在后面持续的跟踪这个想法，和做一些调用（Deepresearch之类的）。




### 统计功能
统计功能能用饼状图展示不同的使用时间类型比例。（基于Timeline的分类）
...
一方面，有用的的知识需要沉淀到知识库（知识库需要暴露API或一个markdown目录）
另一方面，知识库提供接口给client端相关信息（包括相关vector，每个vector有时间戳等meta data）
知识库对比几个方案（obs + 各种插件，腾讯的webxxx，其他）
...
#### 单角色多LLM支持
单个role，有多个LLM Provider支持，或多个LLM支持（fallback场景或并发场景或redundancy场景）
...
### 界面简化
当前界面的功能块都有功能说明（小字），但是其实这些功能描述只需要用户移动鼠标到上面时才需要显示出来，这样界面可以做得更紧凑些。

### 如何能只记录特定屏幕或特定程序窗口？
目前可以区分全屏或激活的窗体进行抓屏。
TBD：只抓屏选择的屏幕，并只当该屏幕被激活时才抓屏。
TBD：排除某些屏幕，
TBD： 排除某些程序windows （可选界面，或直接输入程序名（比如ShuTong）） 
TBD： 可以抓某些屏幕和某些windows。
TBD: 
...
但是，如果用切换到小红书或其他社交媒体看不相关的内容，就可以判断为“分心”。
但是，当用户主动选择进入休闲场景，或从某个时间段开始，用户的使用都是休闲相关（看社交媒体，看新闻，看八卦），那么用户可以后期把相关的内容重新定义为“休闲时刻”，根据多次用户的设定，AI将来能更好的自动设置并进入该“休闲场景”（难判断，容易和分心混淆）


### 知识库
知识库应该外挂，但是ShuTong可以查询，添加补充内容。
知识库应该是本地和网络双备份。本地一份必须是markdown格式，然后网络部分可以考虑github，google drive，Notion，腾讯文档，飞书多备份。使用免费额度即可。
...
**动态调整抓屏间隔**
根据浏览内容，决定更合理抓屏间隔。这个可以作为（auto选项），当用户不选这个时，使用之前指定的策略，选这个时，就是智能策略，并且这个策略会自动学习（？）

**能不能只记录当前处于鼠标激活的window？**（DONE）
这个已经实现，通过使用get-windows库来实现只抓取当前鼠标激活的window。

鼠标是否移到来判断用户是否idle? 比如鼠标超过30秒没有移动，那么就认为用户idle了。从而停止录屏，当鼠标移动了，就重新开始录屏。
...
### 任务列表
创建一个任务列表，用户可以添加任务，任务可以有优先级，任务可以有截止时间，任务可以有完成状态，任务可以有子任务，任务可以有依赖关系，任务可以有提醒，任务可以有提醒时间，任务可以有提醒方式，任务可以有提醒人，任务可以有提醒人邮箱，任务可以有提醒人手机，任务可以有提醒人微信，任务可以有提醒人钉钉，任务可以有提醒人飞书，任务可以有提醒人QQ，任务可以有提醒人微信，任务可以有提醒人.

任务1： 收集关于[xxxx] 的信息
任务2： 写一个Deep Research。 方式一： 后台运行，调用MAS自己实现，用户可以查看过程。方式二：后台通过Playwright MCP调用Gemini Deep Research 网页功能，辅助额外prompt，让gemini网页生成。生成后通知用户。

任务3： Github趋势，热门收集。

任务4： 向知识库添加内容，这个知识库，用户可以添加知识，知识可以有来源，知识可以有标签，知识可以有分类，知识可以有创建时间，知识可以有更新时间，知识可以有创建人，知识可以有更新人，知识可以有创建人邮箱，知识可以有更新人邮箱，知识可以有创建人手机，知识可以有更新人手机，知识可以有创建人微信，知识可以有更新人微信，知识可以有创建人钉钉，知识可以有更新人钉钉，知识可以有创建人飞书，

任务更多：用户需要AI助手可以在后台帮做的事情： 
...
### 两个arguments的agents 的交互方式
对于你提出的课题，这两个agent会以争论的方式讨论。直到得出结论，或到达设置的轮数，用一个评分agent对两者所说的内容进行评价，给出胜者结论。

### Karpathy的多LLM回答并相互评价
多个LLM对同一个问题进行回答，并相互评价，最后由一个评分agent对所有的回复进行评价，给出最终结论。（消耗大量token，建议用户只对重要问题进行使用）。并在开始回答前重写,复述用户的问题(比如给出3个复述，让用户选一个或重新写一个。)

## 6. 新技术尝鲜

### DeepSeek OCR
界面截图，OCR分析，用户和ShuTong进行对答。

### WASM

### WebGPU
Chrome 113+ 支持 WebGPU。如果你的截图需要做图像处理（缩放、色彩校正），可以用 WebGPU Compute Shader，比 Canvas 2D 快 10 倍以上。
