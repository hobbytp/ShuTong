...
export function Sidebar({ activeTab, onTabChange, onBack }: SidebarProps) {
    ...
    return (
        <div className="w-[220px] h-full bg-zinc-950/50 backdrop-blur-xl border-r border-zinc-800 flex flex-col flex-shrink-0 z-20">
            <div className="p-4">
                <button
                    onClick={onBack}
                    className="flex items-center gap-2 text-xs font-medium text-zinc-400 hover:text-white transition-colors group mb-4 px-2"
                >
                ...
                <nav className="space-y-0.5">
                    {tabs.map(tab => (
                        <button
                            ...
                            className={cn(
                                "w-full flex items-center gap-3 px-3 py-2 text-xs rounded-lg transition-all duration-200 group text-left",
                                activeTab === tab.id
                        ...
                        >
                            ...
                            <span>{tab.label}</span>
                            {activeTab === tab.id && (
                                <div className="ml-auto w-1 h-1 rounded-full bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.5)]" />
                            )}
                        </button>
                    ))}
                </nav>
